#!/usr/bin/env bash
# fn-pointer / closure smoke suite.
set -u
...
if [ "${LUNA_NATIVE:-0}" = "1" ]; then
    ...
    OUT_DIR="${OUT_DIR:-/tmp/luna_bootminor_closures}"
else
    LUNA_BOOT="${LUNA_BOOT:-./bootstrap/luna-boot.exe}"
    OUT_DIR="${OUT_DIR:-c:/tmp/luna_bootminor_closures}"
fi
SRC_DIR="$(dirname "$0")"
...
"$LUNA_BOOT" "$full" -o "$OUT_DIR/luna-mini" --target linux \
    || { echo "FAIL: bootstrap did not compile bootminor closures-build"; exit 1; }
cp "$SRC_DIR/tests_closures/"*.luna "$OUT_DIR/"
cp "$SRC_DIR/tests_closures/"*.expect "$OUT_DIR/"
run_linux() {
...
pass=0; fail=0
for src in "$SRC_DIR/tests_closures/"*.luna; do
    name=$(basename "$src" .luna)
    expect_file="$SRC_DIR/tests_closures/$name.expect"
...
echo
echo "=== bootminor closures: $pass PASS, $fail FAIL ==="
exit "$fail"
