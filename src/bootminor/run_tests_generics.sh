...
if [ "${LUNA_NATIVE:-0}" = "1" ]; then
    ...
    OUT_DIR="${OUT_DIR:-/tmp/luna_bootminor_generics}"
else
    LUNA_BOOT="${LUNA_BOOT:-./bootstrap/luna-boot.exe}"
    OUT_DIR="${OUT_DIR:-c:/tmp/luna_bootminor_generics}"
fi
SRC_DIR="$(dirname "$0")"
...
cp "$SRC_DIR/tests_generics/"*.luna "$OUT_DIR/"
cp "$SRC_DIR/tests_generics/"*.expect "$OUT_DIR/"
run_linux() {
...
pass=0; fail=0
for src in "$SRC_DIR/tests_generics/"*.luna; do
    name=$(basename "$src" .luna)
    expect_file="$SRC_DIR/tests_generics/$name.expect"
...
echo
echo "=== bootminor generics: $pass PASS, $fail FAIL ==="
exit "$fail"
