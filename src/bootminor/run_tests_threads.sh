...
if [ "${LUNA_NATIVE:-0}" = "1" ]; then
    ...
    OUT_DIR="${OUT_DIR:-/tmp/luna_bootminor_threads}"
else
    LUNA_BOOT="${LUNA_BOOT:-./bootstrap/luna-boot.exe}"
    OUT_DIR="${OUT_DIR:-c:/tmp/luna_bootminor_threads}"
fi
SRC_DIR="$(dirname "$0")"
...
"$LUNA_BOOT" "$full" -o "$OUT_DIR/luna-mini" --target linux \
    || { echo "FAIL: bootstrap did not compile bootminor threads-build"; exit 1; }
cp "$SRC_DIR/tests_threads/"*.luna "$OUT_DIR/"
cp "$SRC_DIR/tests_threads/"*.expect "$OUT_DIR/"
run_linux() {
...
pass=0; fail=0
for src in "$SRC_DIR/tests_threads/"*.luna; do
    name=$(basename "$src" .luna)
    expect_file="$SRC_DIR/tests_threads/$name.expect"
...
echo
echo "=== bootminor threads: $pass PASS, $fail FAIL ==="
exit "$fail"
