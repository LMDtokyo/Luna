#!/usr/bin/env bash
# M2c smoke suite: user-defined fns + recursion.
set -u
...
if [ "${LUNA_NATIVE:-0}" = "1" ]; then
    ...
    OUT_DIR="${OUT_DIR:-/tmp/luna_bootminor_m2c}"
else
    LUNA_BOOT="${LUNA_BOOT:-./bootstrap/luna-boot.exe}"
    OUT_DIR="${OUT_DIR:-c:/tmp/luna_bootminor_m2c}"
fi
SRC_DIR="$(dirname "$0")"
...
"$LUNA_BOOT" "$full" -o "$OUT_DIR/luna-mini" --target linux \
    || { echo "FAIL: bootstrap did not compile bootminor M2c"; exit 1; }
cp "$SRC_DIR/tests_m2c/"*.luna "$OUT_DIR/"
cp "$SRC_DIR/tests_m2c/"*.expect "$OUT_DIR/"
run_linux() {
...
pass=0; fail=0
for src in "$SRC_DIR/tests_m2c/"*.luna; do
    name=$(basename "$src" .luna)
    expect_file="$SRC_DIR/tests_m2c/$name.expect"
...
echo
echo "=== bootminor M2c: $pass PASS, $fail FAIL ==="
exit "$fail"
