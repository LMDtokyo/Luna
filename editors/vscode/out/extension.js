...
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('luna.runFile', runCurrentFile), vscode.commands.registerCommand('luna.restartLsp', () => restartLspClient(context)), vscode.commands.registerCommand('luna.buildFile', buildCurrentFile), vscode.commands.registerCommand('luna.showOutput', () => outputChannel.show()), vscode.commands.registerCommand('luna.checkFile', checkCurrentFile), vscode.commands.registerCommand('luna.formatFile', formatCurrentFile));
    // Watch for configuration changes
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration('luna.lsp.enabled')) {
            const enabled = vscode.workspace.getConfiguration('luna').get('lsp.enabled', true);
            if (enabled && !client) {
                startLspClient(context);
            }
...
function startLspClient(context) {
    const config = vscode.workspace.getConfiguration('luna');
    const lunaPath = config.get('path', 'luna');
    const trace = config.get('lsp.trace', false);
    ...
    client = new node_1.LanguageClient('lunaLanguageServer', 'Luna Language Server (Pure Luna)', serverOptions, clientOptions);
    ...
    client.start().then(() => {
        ...
        vscode.window.showErrorMessage(`Failed to start Luna Language Server.\n\n` +
...
async function restartLspClient(context) {
